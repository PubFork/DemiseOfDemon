#ifndef BM3_SDK_INC_BM3_CORE_PRIMITIVE_BM3_PRIM_SIMPLEPARTICLE_H
#define BM3_SDK_INC_BM3_CORE_PRIMITIVE_BM3_PRIM_SIMPLEPARTICLE_H
...
/*!
	@brief	シンプルパーティクルエミッタ一つ一つのパラメータの処理
...
//			PrefetchData<sizeof(PrimitiveType)>(prim + 1);
	...
	static void Prepare(PrimitiveType *prim, int active_node_count, int *active_node_index_array, UpdateInfo & /*update_info*/, const ResourceType &res, const obj::RootNodeBase & /*root*/){
		...
		unsigned int type = PIXEL_CASE(res.Blend_Basic_BlendAlphaType(), res.Color_Basic_ColorType());
		switch(type){
			case PIXEL_CASE(BlendAlphaTypeConst_Constant,	ColorTypeConst_Constant):
...
#endif	// #ifndef BM3_SDK_INC_BM3_CORE_PRIMITIVE_BM3_PRIM_SIMPLEPARTICLE_H
