#ifndef BM3_SDK_INC_BM3_CORE_MATRIX_BM3_POLICY_MATRIXROTATE_CURVEVALUE_H
#define BM3_SDK_INC_BM3_CORE_MATRIX_BM3_POLICY_MATRIXROTATE_CURVEVALUE_H
...
/// @brief MatrixRotate_CurveValueƒNƒ‰ƒX
class MatrixRotate_CurveValue{
...
//		if(res.Rotate_Basic_RotateType() == RotateTypeConst_CurveValue)
		{
			value = res.Rotate_Value_Curve().Value(0.0f, primitive_initialize_info.random_seed_, res.Rotate_Value_Range());
...
//		if(res.Rotate_Basic_RotateType() == RotateTypeConst_CurveValue)
		{
			value = res.Rotate_Value_CurveX().Value(0.0f, primitive_initialize_info.random_seed_, res.Rotate_Value_RangeX());
...
//		if(res.Rotate_Basic_RotateType() == RotateTypeConst_CurveValue)
		{
			value = res.Rotate_Value_Curve().Value(primitive_update_info.life_rate_, primitive_update_info.random_seed_, res.Rotate_Value_Range());
			value = value.to_radian();
		}
	}
...
public:
	...
	void BM3_FORCEINLINE Initialize(const ResourceType &res, ml::random &random, ml::vector3d &value, const PrimitiveInitializeInfo &primitive_initialize_info){
		...
		impl::MatrixRotate_CurveValue::Initialize(res, random, value, primitive_initialize_info);
	}
	...
	void BM3_FORCEINLINE Update(UpdateInfo &update_info, const ResourceType &res, ml::vector3d &value, const PrimitiveUpdateInfo &primitive_update_info){
		...
		impl::MatrixRotate_CurveValue::Update(update_info, res, value, primitive_update_info);
	}
	...
	void BM3_FORCEINLINE Prepare(const ResourceType &res, const ml::vector3d &value, ml::matrix44 &m, const PrimitivePrepareInfo &primitive_prepare_info){
		...
		impl::MatrixRotate_CurveValue::Prepare(res, value, m, primitive_prepare_info);
	}
};
...
#endif	// #ifndef BM3_SDK_INC_BM3_CORE_MATRIX_BM3_POLICY_MATRIXROTATE_CURVEVALUE_H
