#ifndef BM3_SDK_INC_BM3_SDK_INC_BM3_CORE_DRAW_DIRECTX9_BM3_DRAW_PARTICLE_H
#define BM3_SDK_INC_BM3_SDK_INC_BM3_CORE_DRAW_DIRECTX9_BM3_DRAW_PARTICLE_H
...
public:
	...
	void	End(DrawInfo &draw_info, ShaderInfo &shader_info, VertexContext &ctx, int child_count, int /*push_count*/, const ResourceType &res){
		...
		EndCore(draw_info, shader_info, EmitterTypeConst_Particle, ctx, child_count, res);
	}
...
#define FixRotateType()		\
	ml::vector3d v0, v1;	\
	MakeVertexVector(v0, v1, prim[active_node_index_array[0]], draw_info);
...
*/
				switch(res.Basic_Meta_OptimizeBit()){
				case OptimizeBit::NoField:
					...
					EndForEach()
				break;
				case OptimizeBit::FixRotate:{
					FixRotateType();
					BeginForEach()
						policy::Position6<DrawScaleType, DrawRotateType>().PushFixRotate(&dst_vertex->position_,	p, draw_info, res, p.GetParentMatrix(), p.GetParentScale(), v0, v1, packet_draw_info,	vertex_element_size_);
						policy::Color6<DrawColorType>().Push(					&dst_vertex->color_,	p, draw_info, res,											packet_draw_info, vertex_element_size_);
						tpf[0](&dst_vertex->texture_[0],	p, draw_info, res, TextureLayer::Type(0),	vertex_element_size_);
						tpf[1](&dst_vertex->texture_[1],	p, draw_info, res, TextureLayer::Type(1),	vertex_element_size_);
					EndForEach()
				...
				break;
				case (OptimizeBit::NoField | OptimizeBit::FixRotate):{
					FixRotateType();
					BeginForEach()
						policy::Position6<DrawScaleType, DrawRotateType>().PushFixRotateNoField(&dst_vertex->position_,	p, draw_info, res, p.GetParentMatrix(), p.GetParentScale(), v0 ,v1, packet_draw_info,	vertex_element_size_);
						policy::Color6<DrawColorType>().Push(					&dst_vertex->color_,	p, draw_info, res,											packet_draw_info, vertex_element_size_);
						tpf[0](&dst_vertex->texture_[0],	p, draw_info, res, TextureLayer::Type(0),	vertex_element_size_);
						tpf[1](&dst_vertex->texture_[1],	p, draw_info, res, TextureLayer::Type(1),	vertex_element_size_);
					EndForEach()
...
*/
				...
				case OptimizeBit::FixRotate:{
					...
					if(res.Blend_Basic_BlendType() == bm3::BlendTypeConst_Add ||res.Blend_Basic_BlendType() == bm3::BlendTypeConst_Sub ){
						BeginForEach()
							policy::Position6<DrawScaleType, DrawRotateType>().PushFixRotateBm2(&dst_vertex->position_,	p, draw_info, res, p.GetParentMatrix(), p.GetParentScale(), v0, v1, packet_draw_info,	vertex_element_size_);
							policy::Color6<DrawColorType>().PushBm2(					&dst_vertex->color_,	p, draw_info, res,											packet_draw_info, vertex_element_size_);
							tpf[0](&dst_vertex->texture_[0],	p, draw_info, res, TextureLayer::Type(0),	vertex_element_size_);
							tpf[1](&dst_vertex->texture_[1],	p, draw_info, res, TextureLayer::Type(1),	vertex_element_size_);
						EndForEach()
				...
				case (OptimizeBit::NoField | OptimizeBit::FixRotate):{
					...
					if(res.Blend_Basic_BlendType() == bm3::BlendTypeConst_Add ||res.Blend_Basic_BlendType() == bm3::BlendTypeConst_Sub ){
						BeginForEach()
							policy::Position6<DrawScaleType, DrawRotateType>().PushFixRotateNoFieldBm2(&dst_vertex->position_,	p, draw_info, res, p.GetParentMatrix(), p.GetParentScale(), v0 ,v1, packet_draw_info,	vertex_element_size_);
							policy::Color6<DrawColorType>().PushBm2(					&dst_vertex->color_,	p, draw_info, res,											packet_draw_info, vertex_element_size_);
							tpf[0](&dst_vertex->texture_[0],	p, draw_info, res, TextureLayer::Type(0),	vertex_element_size_);
							tpf[1](&dst_vertex->texture_[1],	p, draw_info, res, TextureLayer::Type(1),	vertex_element_size_);
						EndForEach()
...
*/
	...
	void MakeVertexVector(ml::vector3d &v0, ml::vector3d &v1, const PrimitiveType &prim, DrawInfo &draw_info){
		float sv0, cv0;
		float sv1, cv1;
		ml::fast::sin_cos( - prim.rotate_.ValueZ() - ml::degree45, sv0, cv0);
		sv1 = -cv0;	cv1 = +sv0;
		v0.e_.x_	= sv0;		v0.e_.y_	= cv0;
		v1.e_.x_	= sv1;		v1.e_.y_	= cv1;
...
#endif	// #ifndef BM3_SDK_INC_BM3_SDK_INC_BM3_CORE_DRAW_DIRECTX9_BM3_DRAW_PARTICLE_H
